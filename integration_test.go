...
func TestFullWorkflowIntegration(t *testing.T) {
	...
	// Should have processed base prefixes plus includes
	expectedOriginal := len(basePrefixes) + len(includePrefixes)
	...
	// Step 8: Verify all prefixes meet minimum length requirements
	ipv4Prefixes := pa.GetIPv4Prefixes()
	ipv6Prefixes := pa.GetIPv6Prefixes()

	for _, prefixStr := range ipv4Prefixes {
		...
		if prefix.Prefix.Bits() < 24 {
			t.Errorf("IPv4 prefix %s has length %d, expected >= 24", prefixStr, prefix.Prefix.Bits())
		}
	}

	for _, prefixStr := range ipv6Prefixes {
		prefix, err := parseIPPrefix(prefixStr)
		if err != nil {
		...
		if prefix.Prefix.Bits() < 48 {
			t.Errorf("IPv6 prefix %s has length %d, expected >= 48", prefixStr, prefix.Prefix.Bits())
		}
	}

	// Step 9: Verify excluded prefixes are not in results
